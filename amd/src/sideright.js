...
define(['jquery', 'core/custom_interaction_events', 'core/log', 'core/pubsub'],
    ...
    var SELECTORS = {
        TOGGLE_REGION: '[data-region="blockregion-toggle"]',
        TOGGLE_ACTION: '[data-action="toggle-blockregion"]',
    ...
    var Drawer = function() {
        ...
        if (!$(SELECTORS.TOGGLE_REGION).length) {
            Log.debug('Page is missing a block region');
        }
        if (!$(SELECTORS.TOGGLE_ACTION).length) {
            Log.debug('Page is missing a blockregion toggle link');
        }
